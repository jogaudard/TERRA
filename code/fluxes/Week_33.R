...
CO2_CH4_1 <- import_CO2_CH4("raw_data/week33/CO2_CH4_2024-08-13.data")
CO2_CH4_2 <- import_CO2_CH4("raw_data/week33/CO2_CH4_2024-08-14.data")
CO2_CH4_3 <- import_CO2_CH4("raw_data/week33/CO2_CH4_2024-08-15.data")
...
PAR_temp_1 <- import_PAR_temp("raw_data/week33/PAR_Temp_2024-08-13.dat")
PAR_temp_2 <- import_PAR_temp("raw_data/week33/PAR_Temp_2024-08-14.dat")
PAR_temp_3 <- import_PAR_temp("raw_data/week33/PAR_Temp_2024-08-15.dat")
...
fieldnotes <- read_csv("raw_data/week33/Fieldnotes_week_33.csv") |>
  mutate(
...
conc_co2_33 <- flux_match(conc_df, fieldnotes, conc_col = "CO2", start_col = "datetime_start", measurement_length = 180, time_diff = -110, startcrop = 20)

conc_ch4_33 <- flux_match(conc_df, fieldnotes, conc_col = "CH4", start_col = "datetime_start", measurement_length = 180, time_diff = -110, startcrop = 20)

conc_co2_33 <- conc_co2_33 |>
  mutate(
...
slopes_co2_33 <- flux_fitting(conc_co2_33, fit_type = "exp")
str(slopes_co2_33)
slopes_ch4_33 <- flux_fitting(conc_ch4_33, fit_type = "exp")

# flux_quality and flux_plot to check the quality and see if we need to modify anything

slopes_co2_33 <- flux_quality(slopes_co2_33, fit_type = "exp")

slopes_ch4_33 <- flux_quality(slopes_ch4_33, fit_type = "exp", ambient_conc = 2000)

flux_plot(slopes_co2_33, f_plotname = "week33_co2", f_ylim_upper = 600, output = "pdfpages")
flux_plot(slopes_ch4_33, f_plotname = "week33_ch4", f_ylim_lower = 1970, f_ylim_upper = 2010, y_text_position = 2000, output = "pdfpages")


# flux_calc to calculate the fluxes
flux_co2_33_chamber <- slopes_co2_33 |>
  filter(TYPE != "C",
...
flux_ch4_33_chamber <- slopes_ch4_33 |>
  filter(
    TYPE != "C") |>
  mutate(
...
flux_co2_33_tube <- slopes_co2_33 |>
  filter(TYPE == "C",
         f_cut == "keep" # we had a cut in quality
  ) |>
  flux_calc(
...
flux_ch4_33_tube <- slopes_ch4_33 |>
  filter(TYPE == "C") |>
  mutate(
    slope_ppm = f_slope_corr * 001 # we need to feed ppm to the function
  ) |>
...
fluxes_33 <- full_join(flux_ch4_33_chamber, flux_co2_33_chamber) |>
  full_join(flux_ch4_33_tube) |>
  full_join(flux_co2_33_tube)
